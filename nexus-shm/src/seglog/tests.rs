...
fn wait_conductor(log: &SegmentedLog) {
    ...
    while log.swap.state() != crate::seglog::conductor::SWAP_CLEAN {
...
fn conductor_builder_custom_queue_depth() {
    ...
    for i in 0u32..40 {
        if log.swap.state() != crate::seglog::conductor::SWAP_CLEAN {
            wait_conductor(&log);
        }
...
fn stress_interleaved_sessions_with_rotation() {
    ...
    for i in 0u32..40 {
        if log1.swap.state() != crate::seglog::conductor::SWAP_CLEAN {
            wait_conductor(&log1);
        }
        if log2.swap.state() != crate::seglog::conductor::SWAP_CLEAN {
            wait_conductor(&log2);
        }
