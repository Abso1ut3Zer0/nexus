...
fn latency_benchmark() {
    ...
    let (mut prod_a, mut cons_a) = nexus_queue::ring_buffer::<u64>(CAPACITY);
    let (mut prod_b, mut cons_b) = nexus_queue::ring_buffer::<u64>(CAPACITY);
...
fn throughput_benchmark() {
    ...
    let (mut producer, mut consumer) = nexus_queue::ring_buffer::<u64>(CAPACITY);
