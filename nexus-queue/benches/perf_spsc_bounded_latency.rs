//! Ping-pong latency benchmark for nexus spsc::bounded (uncached)
//!
//! Measures round-trip latency with exactly one message in flight.
//! Uses uncached variant optimized for low queue depth.
//!
//! Run: cargo build --release --bench perf_latency_spsc_bounded
//! Profile: sudo taskset -c 0,2 ./target/release/deps/perf_latency_spsc_bounded-*
...
fn main() {
    let (mut prod_fwd, mut cons_fwd) = bounded::ring_buffer::<u64>(CAPACITY);
    let (mut prod_ret, mut cons_ret) = bounded::ring_buffer::<u64>(CAPACITY);
